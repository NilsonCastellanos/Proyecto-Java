...
public class CRUDPago {
   ...
   public DefaultTableModel mostrar(String buscar){
       ...
       try {
           ...
           while(rs.next()){
               ...
               registro [5]=rs.getString("precio_total");
   ...
   public boolean insertar (DPago dts){
       sSQL="insert into pago (idreserva,tipo_comprobante,num_comprobante,iva,precio_total,fecha_emision,fecha_pago)" +
               "values (?,?,?,?,?,?,?)";
       try {
   ...
   public boolean editar (DPago dts){
       sSQL="update pago set idreserva=?,tipo_comprobante=?,num_comprobante=?,iva=?,precio_total=?,fecha_emision=?,fecha_pago=?"+
               " where idpago=?";
